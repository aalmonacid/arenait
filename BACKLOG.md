...
- **Criterios de Aceptación**:
  - [ ] Repositorio inicializado con la estructura base y archivos de contexto (AGENTS.md, README.md).
  - [ ] Configuración de Astro 5.0 con Tailwind CSS (`astro.config.mjs`, `tailwind.config.mjs`).
  - [ ] Setup de cliente Sanity (`src/lib/sanity.ts`) con tipado correcto (`src/env.d.ts`).
  - [ ] Tokens de diseño corporativos (Swiss Minimalist) implementados en configuración Tailwind.

## Sprint 1: Integración CMS y Estructura Base (UI Shell)
- **Objetivo**: Conexión de backend headless y estructura principal de UI.
- **Criterios de Aceptación**:
  - [ ] Esquemas de contenido base en Sanity.io para páginas clave (Project ID `xbayv7k2`).
  - [ ] Recuperación de datos desde Astro utilizando el cliente de Sanity.
  - [ ] Implementación de un Layout global (Header, Footer, Meta-tags SEO).
  - [ ] Aplicación estricta de tipografías y reglas de contraste (WCAG AAA).
