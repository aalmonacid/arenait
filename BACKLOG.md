...
- [ ] **Fondo del hero oscuro — decisión de marca pendiente.** `bg-neutral-900` (#0F172A) sigue en uso en 6+ lugares (home, servicios, casos de estudio, nosotros, contacto, footer) — confirmado previamente como deliberado, no un bug. El 2026-09-03 se generó una comparación visual (`#0F172A` actual vs. `#294E6C` secundario de marca) para que el cliente decida; no se tocó código. Ver artefacto de comparación enlazado en la conversación — si el cliente confirma el cambio, reemplazar `bg-neutral-900` por `bg-secondary` en los 6 archivos listados.
